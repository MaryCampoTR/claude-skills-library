...
description: >
  ...
  export, or any UI image and needs it interpreted into Saffron component
...
### Step 2: Map visible elements to Saffron components

For each distinct UI element visible, attempt to identify it against the
Saffron component vocabulary. Use this matching hierarchy:

1. **Exact match** — element clearly matches a known Saffron component
2. **Probable match** — element likely matches with minor uncertainty
3. **Unrecognised** — element does not match any known Saffron component
   (flag as [CUSTOM] or [UNRESOLVED])

**Known Saffron components to match against:**
...
- Primary heading (font size/weight → Clario heading scale)
...
| Visual appearance | Likely Saffron component |
|---|---|
